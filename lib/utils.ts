...
export function clampPage(value: string | null, fallback: number, totalPages: number) { const parsed = parsePositiveInt(value, fallback); return Math.min(Math.max(parsed, 1), Math.max(totalPages, 1)); }
